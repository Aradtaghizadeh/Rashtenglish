...
function validateAnswers(answers) {
  const correctVocab = ['tiny', 'a', 'important', 'reduce', 'run'];
  const correctGrammar = ['had', 'had studied', 'will have finished', 'was he', 'spoke'];
  const correctReading = ['global warming', 'extreme weather conditions', 'carbon emissions', 'to reduce carbon emissions', 'global warming'];
  const correctListening = ['solar and wind', 'they help reduce carbon emissions and combat climate change', 'carbon emissions', 'solar power', 'optimistic'];
  ...
  try {
    ...
    if (writingText) {
      ...
      if (words >= 150 && words <= 180) {
        ...
        writingFeedback += `Word count invalid (${words}; must 150-180, 0 pts). `;
      }

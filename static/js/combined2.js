...
function validateAnswers(answers) {
  const correctVocab = ['hard', 'doctor', 'interesting', 'pharmacy', 'never'];
  const correctGrammar = ['had', 'went', 'since', 'to sleeping', 'was watching'];
  const correctReading = ['easier and often cheaper', 'cannot see the product before buying', 'cheaper', 'no', 'online shopping'];
  const correctListening = ['time they are meeting', 'where they are meeting', 'what they want to drink', 'who is coming with them', 'morning or evening'];
  ...
  try {
    ...
    if (writingText) {
      ...
      if (words >= 100 && words <= 120) {
        writingScore += 10;
        writingFeedback += 'Word count OK (10 pts). ';
      } else {
        writingFeedback += `Word count invalid (${words}; must 100-120, 0 pts). `;
      }
